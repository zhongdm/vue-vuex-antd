<template>
  ...
  <a-menu
    ...
    <a-menu-item v-else :key="item.key" data-path="/settings">{{item.value}}</a-menu-item>
    </template>
  </a-menu>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleMenu (e) {
      ...
      // this.$route.push(e.domEvent.target.dataset.path)
      console.log('selected', e.domEvent.dataset.path)
    }
  }
}
</script>
